...
Goals:
- Handle PDF, DOCX, XLSX/XLS/CSV, images (PNG/JPG), TXT, and fall-through for unknown.
- PDFs: if <= ~32 MB and few pages → send as base64 document block (Claude's native PDF).
  Otherwise extract text with pdfplumber and render first ~10 pages as images.
- Excel: convert each sheet to a compact text table.
- DOCX: extract plain text.
- Images: pass through as image blocks (base64).
- Emit per-file summary lines so Claude can cite sources.
"""
...
def _ingest_pdf(filename: str, data: bytes) -> IngestedFile:
    ...
    if size <= MAX_PDF_DIRECT_BYTES:
        ...
        return out
    # Fallback: extract text + render first N pages
    try:
        import pdfplumber
        ...
        out.content_blocks.append({"type": "text", "text": f"[PDF {filename} — large, text-extracted from {total_pages} pages]\n\n{joined}"})
...
def _ingest_image(filename: str, data: bytes, media_type: str) -> IngestedFile:
    ...
    if len(data) > MAX_IMG_BYTES:
        try:
            ...
            im.convert("RGB" if fmt == "JPEG" else "RGBA").save(buf, format=fmt, quality=85 if fmt == "JPEG" else None)
            data = buf.getvalue()
...
def ingest_file(filename: str, data: bytes) -> IngestedFile:
    """Route a single uploaded file to the right handler by extension/mime."""
    ...
    if ext in (".png",) or mime == "image/png":
    ...
    if ext in (".gif",) or mime == "image/gif":
        return _ingest_image(filename, data, "image/gif")
    if ext in (".webp",) or mime == "image/webp":
        return _ingest_image(filename, data, "image/webp")
    if ext in (".txt", ".md", ".log"):
        return _ingest_text(filename, data)
    # Fallback: describe the unknown
    out = IngestedFile(filename=filename)
    out.content_blocks.append({"type": "text", "text": f"[UNSUPPORTED {filename} — {len(data)/1024:.0f} KB, ext={ext!r}] Skipped content extraction; ask the user about this file in your unknowns list."})
