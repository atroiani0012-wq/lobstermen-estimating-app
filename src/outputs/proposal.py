"""
Output 4 — First draft Cost Proposal (Word doc).
"""
...
def build_proposal_docx(analysis: dict[str, Any]) -> bytes:
    ...
    _add_para(doc, f"Prepared by: UMA")
    ...
    if takeoff:
        _add_table(
            ...
            [[t.get("item", ""), t.get("unit", ""), t.get("quantity", 0),
    ...
    if testing:
        _add_table(doc, ["Test", "Unit", "Qty", "Notes"],
                   [[t.get("test_type", ""), t.get("unit", ""), t.get("quantity", 0), t.get("notes", "")] for t in testing])
    else:
