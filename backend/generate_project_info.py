"""Generate 02_Project_Info.docx — project summary brief."""
...
def build_project_info_docx(analysis: dict[str, Any]) -> bytes:
    ...
    if appendix:
        _add_table(
            doc,
            ["File", "Type", "Summary", "Key info"],
            [
                [f.get("filename", ""), f.get("type", ""), f.get("summary", ""), f.get("key_info", "")]
                for f in appendix
            ],
        )
    else:
    ...
    if testing:
        _add_table(
            doc,
            ["Test", "Unit", "Qty", "Spec Section", "Notes"],
            [
                [
                    t.get("test_type", ""),
                    t.get("unit", ""),
                    t.get("quantity", 0),
                    t.get("reference_spec_section", ""),
                    t.get("notes", ""),
                ]
                for t in testing
            ],
    ...
    if ua:
        _add_table(
            doc,
            ["Unknown", "Assumption", "Reasoning", "Impact if wrong"],
            [
                [
                    u.get("item", ""),
                    u.get("assumption", ""),
                    u.get("reasoning", ""),
                    u.get("impact_if_wrong", ""),
                ]
                for u in ua
            ],
        )
    ...
    if vendors:
        _add_table(
            doc,
            ["Category", "Suggested Vendors", "# takeoff items", "# testing items"],
            [
                [
                    v.get("vendor_category", ""),
                    ", ".join(v.get("suggested_vendors") or []),
                    len(v.get("takeoff_items_for_rfq") or []),
                    len(v.get("testing_items_for_rfq") or []),
                ]
                for v in vendors
            ],
        )
